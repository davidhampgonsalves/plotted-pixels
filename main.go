...
func main() {
  img := Load("test.full.png")

  bitmask := GenerateBitmask(img, color.NRGBA{0x18, 0x18, 0x18, 0xff})
  w := 1000
  pixelSize := float64(w) / float64(len(bitmask[0]))
  linesPerPixel := 5
  ...
  config := drawconfig{width: w,
    height: int(pixelSize * float64(len(bitmask))),
  ...
  Draw(gc, config, remainingBitmask)
...
func Draw(gc *draw2dsvg.GraphicContext, c drawconfig, remainingBitmask [][]bool) {
	gc.SetStrokeColor(color.RGBA{0xAA, 0xAA, 0xFF, 0xFF})
...
func NextLinePosition(bitmask [][]bool, movementOrder []string, x, y int) []int {
  for _, movement := range movementOrder {
    switch movement {
      case "left":
        if(IsValidPosition(bitmask, x-1, y)) { return []int{x-1, y} }
      case "up":
        if(IsValidPosition(bitmask, x, y-1)) { return []int{x, y-1} }
      case "down":
        if(IsValidPosition(bitmask, x, y+1)) { return []int{x, y+1} }
      case "right":
        if(IsValidPosition(bitmask, x+1, y)) { return []int{x+1, y} }
  ...
  if(IsValidPosition(bitmask, x+1, y+1)) { return []int{x+1, y+1} }
  if(IsValidPosition(bitmask, x-1, y+1)) { return []int{x-1, y+1} }
  if(IsValidPosition(bitmask, x-1, y-1)) { return []int{x-1, y-1} }
  if(IsValidPosition(bitmask, x+1, y-1)) { return []int{x+1, y-1} }
...
func IsValidPosition(bitmask [][]bool, x, y int) bool {
  if(y < 0 || y >= len(bitmask)) { return false }
  if(x < 0 || x >= len(bitmask[y])) { return false }
